...
loclinear.mean<-cv4abc(param=t(prior2), sumstat=t(pred2), nval=100, tols=tol, statistic = "median", method="loclinear")
...
bpval.model<-gfit(target=stoat.trapping$stoat.capt, sumstat=t(pred2), statistic=median, nb.replicate=200)

summary(bpval.model)

#### Inference

inf.abc<-abc(target=stoat.trapping$stoat.capt, param=t(prior2), sumstat=t(pred2), tol=0.1, method="loclinear")
