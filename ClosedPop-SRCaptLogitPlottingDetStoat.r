...
### Plotting the posterior estimate of the probability of capture of stoats as a function of the trapping effort
set.seed(2012)
...
rem.data<-read.table("C:/Orkney/Model/Removal-SR2.csv", sep=",", header=TRUE)
...
eff.stoat<-(rem.data$Total.traps-rem.data$stoat-rem.data$closed.trap)*rem.data$Days		 
...
rem.conf<-configureMCMC(Rmodel,  monitors=list('p.stoat'), thin=10)
...
av.det<-colMeans(out.post)

quant.det<-apply(out.post, 2, function(x) quantile(x, c(0.0275, 0.975)))

quant.det

plot(av.det~log10(eff.stoat), type="l", lwd=4, ylim=c(0, 1), main="Stoat probability of capture", ylab="Probability of capture",
