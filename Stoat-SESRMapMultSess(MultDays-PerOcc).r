...
### All potential stoat home range centres
...
#### Transform to matrix containing potential home range centres
hr.pot<-matrix(0, ncol=2, nrow=pot.stoat$n)

hr.pot[, 1]<-pot.stoat$x

hr.pot[, 2]<-pot.stoat$y
...
se.stoat<-function(x, stoat.trapping, eff.stoat, hr.pot){
    ...
    samp.hr<-sample(c(1:nrow(hr.pot)), z, replace=FALSE)           ### Select home range centres at random

    stoat.pos<-hr.pot[samp.hr, ]                            ### Stoat home ranges
...
write.table(post.pred, "d:/CONTAIN/Stoats RSPB/Modelling results/Inference-ModelSETrap2.csv", sep=",")
