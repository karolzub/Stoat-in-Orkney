...
source("C:/Orkney/Model/StoatSim_Dist_Main.R")

stoat.hr<-cmpfun(stoat.dist)

#### Load the trap data (trap positions)
main <- readOGR("C:/Orkney/Model/Main.shp")
#access <- readOGR("C:/Orkney/Model/Main_access1.shp")
#plot(main)
#n.trap<-length(centre.trap$coords.x1)
#trap.pos<-as.data.frame(trap.pos)

#### Load the trap data (trap positions)
traps_bord <- readOGR("C:/Orkney/Model/Traps_on_border.shp")
n.trap<-length(centre.trap$coords.x1)
#traps_bord<-as.data.frame(traps_bord)
#plot(traps_bord)

############# SIMULATIONS
### Define the study area (same across simulations)
#main<-readOGR("C:/Orkney/Model/SR_area.shp")
...
n.day<-round(qunif(lhs1[,1], 1, 40), digits=0)

init.density<-round(qunif(lhs1[,2], 0.01, 0.1), digits=4)

n.trap<-round(qunif(lhs1[,3], 1000, 5000), digits=0)           ### Total number of traps

trap.dist<-round(qunif(lhs1[,4], 50, 500), digits=0)           ### Minimum distance between traps

g0<-round(qunif(lhs1[,5], 0.01, 0.1), digits=4)           ### Probability of capture at the centre of the home range

sigma<-round(qunif(lhs1[,6], 200, 700), digits=0)           ### Spatial decay parameter

p.bycatch<-round(qunif(lhs1[,7], 0.005, 0.015), digits=5)           ### Probability of by-catch per trap day
...
n.trap<-round(qunif(lhs1[,3], 250, 1000), digits=0)           ### Total number of traps
...
sens.data <- data.frame(prop.stoat=stoat.sim[1, ], n.day = n.day, init.density = init.density, n.trap = n.trap, trap.dist= trap.dist, g0=g0, sigma = sigma, p.bycatch = p.bycatch)

sens.mod<-gbm.step(sens.data, gbm.x=c(2:8), gbm.y=1, tree.complexity=3, learning.rate=0.0001, bag.fraction=0.7, step.size=100, max.trees=10000,  n.folds=10, family="gaussian")
