...
source("C:/Orkney/Frontiers/Model1_1.R")
...
n.trap<-round(qunif(lhs1[,3], 1000, 10000), digits=0)           ### Total number of traps

trap.dist<-round(qunif(lhs1[,4], 50, 250), digits=0)    ### Minimum distance between traps (only for simulated traps)
...
plot(stoat.sim[2, ]~n.day, xlab="Number of days", ylab="Percentage caught")
...
plot(stoat.sim[2, ]~g0, xlab="Probability of capture", ylab="Percentage caught")

plot(stoat.sim[2, ]~sigma, xlab="Spatial decay parameter", ylab="Percentage caught")

plot(stoat.sim[2, ]~p.bycatch, xlab="Probability of a by-catch", ylab="Percentage caught")
...
sens.data <- data.frame(prop.stoat=stoat.sim[1, ], n.day = n.day, init.density = init.density, trap.dist= trap.dist, n.trap=n.trap, g0=g0, sigma = sigma, p.bycatch=p.bycatch)

sens.mod<-gbm.step(sens.data, gbm.x=c(2:7), gbm.y=1, tree.complexity=3, learning.rate=0.001, bag.fraction=0.7, step.size=100, max.trees=10000,  n.folds=10, family="gaussian")

summary(sens.mod)

plot(sens.mod,6)  ### plot for different parameters indicated by number after comma
