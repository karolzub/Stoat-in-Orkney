...
source("C:/Orkney/Model/StoatSim_Dist_Orkney.R")
...
trap.dist<-round(qunif(lhs1[,3], 50, 250), digits=0)    ### Minimum distance between traps (only for simulated traps)

g0<-round(qunif(lhs1[,4], 0.01, 0.1), digits=4)     ### Probability of capture at the centre of the home range

sigma<-round(qunif(lhs1[,5], 100, 700), digits=0)    ### Spatial decay parameter
...
sens.data <- data.frame(prop.stoat=stoat.sim[1, ], n.day = n.day, init.density = init.density, trap.dist= trap.dist, g0=g0, sigma = sigma, p.bycatch=p.bycatch)
...
plot(sens.mod,5)  ### plot for different parameters indicated by number after comma
