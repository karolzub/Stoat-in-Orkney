...
se.stoat<-function(x){
    ...
    out.sim1<-c(unlist(stoat.cap1))

    norm.sim1<-(out.sim1-mean(out.sim1))/(sd(out.sim1)/sqrt(length(out.sim1)))
    
    out.sim2<-c(unlist(stoat.cap2))

    norm.sim2<-(out.sim2-mean(out.sim2))/(sd(out.sim2)/sqrt(length(out.sim2)))

    return(c(out.sim1, out.sim2))
}



########## Summary statistics to target
sum.stat1<-stoat.trapping$stoat.capt

sum.stat1

norm.stat1<-(sum.stat1-mean(sum.stat1))/(sd(sum.stat1)/sqrt(length(sum.stat1)))

sum.stat2<-as.numeric(table(stoat.trapping$stoat.capt, stoat.trapping$VISIT_NO)[2,])

sum.stat2

norm.stat2<-(sum.stat2-mean(sum.stat2))/(sd(sum.stat2)/sqrt(length(sum.stat2)))

norm.stat2

sum.stat<-c(norm.stat1, norm.stat2)
