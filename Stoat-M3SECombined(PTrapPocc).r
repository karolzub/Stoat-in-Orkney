...
sigma<-round(qnorm(lhs1[,2], 255, 100), digits=2)
...
pred.vector<-matrix(0, nrow=length(stoat.trapping$stoat.capt)+n.session, ncol=n.its)
...
for (i in 1:n.its){
    ...
    pred.vector[ , i]<-tryCatch(se.stoat(prior.vector[, i], stoat.trapping, eff.stoat, sr=sr), error=function(e) NA)   ### Storing the summary statistics
...
loclinear.mean<-cv4abc(param=t(prior3), sumstat=t(pred3), nval=100, tols=c(0.1, 0.5, 1), statistic = "median", method="loclinear")

ridge.mean<-cv4abc(param=t(prior3), sumstat=t(pred3), nval=100, tols=tol, statistic = "mean", method="ridge")

rejection.mean<-cv4abc(param=t(prior3), sumstat=t(pred3), nval=100, tols=tol, statistic = "mean", method="rejection")

nnet.mean<-cv4abc(param=t(prior3), sumstat=t(pred3), nval=2, tols=c(0.1, 0.5, 1), statistic = "median", method="neuralnet")
...
bpval.model<-gfit(target=sum.stat, sumstat=t(pred3), statistic=mean, nb.replicate=200)

summary(bpval.model)

#### Inference

inf.abc<-abc(target=sum.stat, param=t(prior3), sumstat=t(pred3), tol=0.1, method="rejection")
...
write.table(post.pred, "d:/CONTAIN/Stoats RSPB/Modelling results/Inference-ModelSETrapCombined.csv", sep=",")
