...
source("C:/Orkney/Frontiers/Model3_1SR.R")

stoat.hr<-cmpfun(stoat.dist)

#### Load the trap data (trap positions)
trap.pos<-readOGR("C:/Orkney/Model/SR_traps.shp")
...
tot.area<-sum(SR$SR_area)/10000                    ### Area covered in ha (SR)
#plot(orkney)
...
init.density<-round(qunif(lhs1[,2], 0.014, 0.019), digits=4)    ### initial density of stoats per ha
