...
### Packages required.
        ...
        centre.stoat.rand <- spsample(sr, n = tot.stoat, "random", iter = 20)
        centre.stoat <- as.data.frame(centre.stoat.rand)
        
        ### All potential trap locations
        #trap.pos<-readOGR("C:/Orkney/Model/TrapLocations.shp")
        centre.trap.rand <- spsample(sr, n = n.trap, "random", iter = 20)
       ...
       ### Simulate stoat positions during n.days
        ...
        stoat.rem<-tot.stoat-sum(stoat.camera)

        ### Output of the function - returns the mean home range size (ha), the percentage of stoats caught, and the number of stoats remaining and not caught

        out.sim<-round(c(mean(hr.area$area), perc.catch, stoat.rem), digits=2)
