...
lhs1<-randomLHS(n.sim, 3)

g0<-qunif(lhs1[,1], -5, 5)

sigma<-round(qunif(lhs1[,2], 2, 7), digits=2)

n.tot<-round(qunif(lhs1[,3], 3.5, 6.5), digits=1)           ### Minimum distance between traps

### First set a progress bar
pb<-txtProgressBar(min = 0, max = n.sim, style = 3)

### Empty matrices to store the results of the simulations
pred.vector<-matrix(0, nrow=length(stoat.trapping$stoat.capt), ncol=n.sim)

prior.vector<-matrix(0, nrow=3, ncol=n.sim)
...
### Exclude all 0s
id.zero<-which(colSums(pred2)==0)

id.zero

pred3<-pred2[ , -id.zero]                                  ### Exclude simulations that produce NAs

ncol(pred3)

ncol(pred3)/n.sim

prior3<-prior2[ , -id.zero]
...
bpval.model<-gfit(target=stoat.trapping$stoat.capt, sumstat=t(pred3), statistic=median, nb.replicate=200)

summary(bpval.model)

#### Inference

inf.abc<-abc(target=stoat.trapping$stoat.capt, param=t(prior3), sumstat=t(pred3), tol=1, method="rejection")
