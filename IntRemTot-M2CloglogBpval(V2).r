...
rem.model<-nimbleCode({
    ...
    for (s in 1:n.site){
        ...
        stoat.obs[s]~dpois(mult.obs*n.stoat[s])

        ##### Bayesian p-values
		
		###### Stoats
		exp.stoat[s]<-mult.obs*n.stoat[s]

		obs.stoat[s]<-pow(sqrt(stoat.obs[s])*sqrt(exp.stoat[s]), 2)

        new.stoat[s]~dpois(mult.obs*n.stoat[s])

		sim.stoat[s]<-pow(sqrt(new.stoat[s])*sqrt(exp.stoat[s]), 2)		

    }

        #### Freeman-Tukey test for each site

		#### Stoat

		fto.stoat<-sum(obs.stoat[1:n.site])
	
		fts.stoat<-sum(sim.stoat[1:n.site])
...
rem.conf<-configureMCMC(Rmodel,  monitors=list('bp.stoat'), thin=1)
