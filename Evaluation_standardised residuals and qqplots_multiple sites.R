...
rem.data<-read.table("C:/Orkney/Model/Removal_sites.csv", sep=";", header=TRUE)
...
n.init<-max(apply(stoat.capt, 1, function(x) sum(x, na.rm=TRUE)))*2
...
rem.conf<-configureMCMC(Rmodel,  monitors=list('exp.stoat', 'new.stoat'), thin=10)
...
rem.m2<-runMCMC(Cmcmc, niter=500000, nchains=3, nburnin=10000, inits=inits, samplesAsCodaMCMC=FALSE)
