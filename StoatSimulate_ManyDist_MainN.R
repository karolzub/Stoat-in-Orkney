...
source("C:/Orkney/Model/StoatSim_Dist_MainN.R")
...
#tot.area<-sum(access$Area3)/10000 
#plot(main)
...
traps_rand <- readOGR("C:/Orkney/Model/Main_all_random_points.shp")
#n.trap<-length(centre.trap$coords.x1)
#traps_bord<-as.data.frame(traps_bord)
n.trap<-length(traps_rand$coords.x1)
#plot(traps_bord, add = TRUE)
traps_rand<-as.data.frame(traps_rand)
