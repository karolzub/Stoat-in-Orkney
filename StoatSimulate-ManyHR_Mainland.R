...
source("C:/Orkney/Model/StoatSimHR_Mainland.R")
...
init.density<-round(qunif(lhs1[,2], 0.01, 1), digits=3)
...
q.arg <- list(list(min=7, max=40),list(min=0.01, max=0.1), list(min=1000, max=7000),list(min=50, max=250),
...
n.trap<-round(qunif(lhs1[,3], 100, 7000), digits=0)           ### Total number of traps
trap.dist<-round(qunif(lhs1[,4], 50, 250), digits=0)           ### Minimum distance between traps
cor.mov<-round(qunif(lhs1[,5], 0.001, 0.8), digits=3)           ### Position correlation between days across stoats
max.det<-round(qunif(lhs1[,6], 0.11, 0.8), digits=3)           ### Maximum probability of capture per night (across stoats)
p.bycatch<-round(qunif(lhs1[,7], 0.005, 0.015), digits=5) 
...
sens.mod<-gbm.step(sens.data, gbm.x=c(2:9), gbm.y=1, tree.complexity=3, learning.rate=0.001, bag.fraction=0.7, step.size=1000, max.trees=10000,  n.folds=10, family="gaussian")
...
newdat = data.frame(hr = seq(min(DataDF$hr), max(DataDF$hr), length.out = 100))
newdat$pred = predict(fit, newdata = newdat)

plot(Perc_caught ~ hr, xlab="Average stoat home range (ha)", ylab="Percentage caught", data = DataDF)
with(newdat, lines(x = hr, y = pred, col = "blue"))
