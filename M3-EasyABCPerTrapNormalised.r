...
se.stoat<-function(x){
    ...
    out.sim<-c(unlist(stoat.cap1))

    norm.out<-(out.sim-mean(out.sim))/(sd(out.sim)/sqrt(length(out.sim)))
...
norm.sum<-(sum.stat-mean(sum.stat))/(sd(sum.stat)/sqrt(length(sum.stat)))
...
### ABC algorithm
ABC.mcmc<-ABC_mcmc(method="Marjoram_original", model=se.stoat, prior=prior.list, summary_stat_targetnorm.sum, n_rec=n.its, progress_bar=TRUE,
    dist_max=0.01)


ABC.rej<-ABC_rejection(model=se.stoat, prior=prior.list, nb_simul=n.its, summary_stat_target=norm.sum, tol=0.01, 
            progress_bar=TRUE)
