...
se.stoat<-function(x, stoat.trapping){
    ...
    ##### Observation model - distances between home ranges and each trap and probability of capture
	        ...
	        for (i in 1:av.stoat){
    			...
    			comp.eff[i, j]<-1-(1-p.capt[i, j])^stoat.analys$DAYS_FROM_PREV_VISIT[j]		#### Complement of the effective capture prob
...
for (i in 1:n.its){
    ...
    prior.vector[, i]<-c(runif(1, 0, 1), runif(1, 0, 1000), runif(1, 0, 1), runif(1, 1, 500))       #### Priors
...
loclinear.mean<-cv4abc(param=t(prior2), sumstat=t(pred2), nval=2, tols=1, statistic = "median", method="loclinear")
