...
sum.stat2<-stoat.trapping$stoat.capt
...
abc.pf<-ABC_sequential(method="Lenormand", model=se.stoat, prior=prior.list, nb_simul=n.its, summary_stat_target=sum.stat2,
            progress_bar=TRUE, alpha=0.1)

colMeans(abc.pf$stats)

plot(sum.stat2, colMeans(abc.pf$stats), xlab="True value of the summary statistics", 
...
############################ Bayesian p-value
bpval.model<-gfit(target=sum.stat2, sumstat=abc.pf$stats, statistic=median, nb.replicate=200)
