...
source("C:/Orkney/Model/StoatSim_Dist_Deerness2.R")

stoat.hr<-cmpfun(stoat.dist)

#### Load the trap data (trap positions)
trap.pos<-readOGR("C:/Orkney/Model/Deerness_traps.shp")
n.trap<-length(trap.pos$TRAP_ID)
trap.pos<-as.data.frame(trap.pos)


############# SIMULATIONS
### Define the study area (same across simulations)
deerness<-readOGR("C:/Orkney/Model/Deerness.shp")
tot.area<-sum(deerness$DN_area)/10000                     ### Area covered in ha (Deerness)
