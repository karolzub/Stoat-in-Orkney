...
### Coordinates of the island - divide it into 500 x 500-metre cells
x.coord<-seq(min(stoat.trapping$X)-10, max(stoat.trapping$X)+10, by=500)

x.min<-min(x.coord)

x.max<-max(x.coord)

y.coord<-seq(min(stoat.trapping$Y)-10, max(stoat.trapping$Y)+10, by=500)

y.min<-min(y.coord)

y.max<-max(y.coord)

pos.coord<-expand.grid(X=x.coord, Y=y.coord)
...
se.stoat<-function(x, stoat.trapping, eff.stoat, pot.coord){
    ...
    samp.hr<-sample(nrow(pot.coord), z, replace=TRUE)

    stoat.pos<-pot.coord[samp.hr, ]
...
pred.vector<-matrix(0, nrow=n.session, ncol=n.its)
...
for (i in 1:n.its){
    ...
    pred.vector[ , i]<-tryCatch(se.stoat(prior.vector[, i], stoat.trapping, eff.stoat, pot.coord=pos.coord), error=function(e) NA)   ### Storing the summary statistics
...
plot(loclinear.mean$true~loclinear.mean$estim)
...
inf.abc<-abc(target=sum.stat, param=t(prior3), sumstat=t(pred3), tol=0.1, method="loclinear")
...
write.table(post.pred, "d:/CONTAIN/Stoats RSPB/Modelling results/Inference-ModelSETrapPerOcc.csv", sep=",")
