...
### Packages required.
        ...
        ### Trap random locations on properties borders 
        #trapsb <- sample_n(traps_bord, size = n.trap, replace = FALSE)
        #xtrap <- trapsb$coords.x1                      
        #ytrap <- trapsb$coords.x2
        #trap.loc <-cbind(xtrap,ytrap)
        
        ### Trap random locations on access land and properties borders with no access
        trapsb <- sample_n(traps_rand, size = n.trap, replace = FALSE)
        xtrap <- trapsb$coords.x1                      
        ytrap <- trapsb$coords.x2
        trap.loc <-cbind(xtrap,ytrap)
